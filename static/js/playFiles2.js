...
function startBackgroundTrack() {
    if (!backgroundTrack) {
        backgroundTrack = new Tone.Player({
            url: "static/audio/background2.mp3", // background track for group 1
            autostart: false,
            loop: true,
            onload: () => {
                console.log("background track 2 loaded successfully.");
                backgroundTrack.start();
                console.log("background track 2 started playing.");
            },
            onerror: (error) => {
                console.error("error loading background track 2:", error);
            }
        }).toDestination();
    } else {
        backgroundTrack.start();
